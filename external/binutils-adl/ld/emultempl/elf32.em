...
{
  extern const char *adl_arch_name;
  ldfile_set_output_arch (adl_arch_name, bfd_arch_adl);
...
if test x"$GENERATE_ACB_OPTION_HOOK" = xyes; then
fragment <<EOF
  void acb_add_options(int nl, struct option **longopts);

  acb_add_options(nl + (sizeof(xtra_long)/sizeof(struct option)),longopts);
EOF
fi
...
fragment <<EOF
    {
      int acb_handle_option(int,const char *);
      return acb_handle_option(optc,optarg);
    }
EOF
fi
if test x"$GENERATE_ACB_OPTION_HOOK" != xyes; then
...
if test x"$GENERATE_ACB_OPTION_HOOK" = xyes; then
fragment <<EOF
  void acb_list_options(FILE *file);
  acb_list_options(file);
EOF
fi
if test x"$GENERATE_SHLIB_SCRIPT" = xyes; then
fragment <<EOF
