...
// Hack to setup emulation names properly.
static void adl_setup_emul_names(void)
{
  extern char *adl_emul_names[];

  ld_emulations[0]->emulation_name = adl_emul_names[0];
  ld_emulations[0]->target_name    = adl_emul_names[1];
  ld_emulations[1]->emulation_name = adl_emul_names[2];
  ld_emulations[1]->target_name    = adl_emul_names[3];  
}

// Another ADL hack.  If we're using on of the default ADL formats,
// then switch to the equivalent format that's supported by BFD.
static void adl_update_output_target(void)
{
  extern const char *adl_default_targ32;
  extern const char *adl_default_targ64;
  
  if (output_target != NULL) {
    if (strcmp(output_target,"elf32-adl") == 0) {
      // Find first 32-bit BFD.
      output_target = adl_default_targ32;
    } else if (strcmp(output_target,"elf64-adl") == 0) {
      // Find first 64-bit BFD.
      output_target = adl_default_targ64;
    }
  }
}

// ADL option processing hooks.

int __attribute__((weak)) acb_handle_option(int c,const char *arg);

int acb_handle_option(int c  ATTRIBUTE_UNUSED,const char *arg  ATTRIBUTE_UNUSED)
{
  return FALSE;
}

void __attribute__((weak)) acb_add_options(int nl,struct option **longopts);

void acb_add_options(int nl  ATTRIBUTE_UNUSED,struct option **longopts  ATTRIBUTE_UNUSED)
{
}                        

void __attribute__((weak)) acb_list_options(FILE *);

void acb_list_options(FILE *file ATTRIBUTE_UNUSED)
{
}                        
...
main (int argc, char **argv)
{
  ...
  extern const char *adl_default_target;
  
  if (! bfd_set_default_target (adl_default_target))
    {
...
extern const char *adl_default_emul;
...
get_emulation (int argc, char **argv)
{
  ...
  if (emulation == NULL)
    ...
    emulation = (char*)adl_default_emul;
