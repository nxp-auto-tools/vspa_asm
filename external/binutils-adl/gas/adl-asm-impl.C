...
static const adl_int_pair         *reloc_offsets[NUM_CORES] = { 0, 0 };
static int                         num_relocs[NUM_CORES] = { 0, 0 };
static int                         num_relocs_by_index[NUM_CORES] = { 0, 0 };
static int                         num_reloc_offsets[NUM_CORES] = { 0, 0 };
                           ...
                           const adl_int_pair *reloc_offsets_p,int num_reloc_offsets_p)
{
  ...
  reloc_offsets[init_core]     = reloc_offsets_p;
  num_reloc_offsets[init_core] = num_reloc_offsets_p;
}
...
{
  acb_setup_finish(all_instrs_hash[core],all_fields[core],num_fields[core],relocs[core],num_relocs[core]);
}
...
const reloc_howto_type *adl_reloc_type_lookup(bfd *abfd, bfd_reloc_code_real_type code)
{
  if (relocs[core]) {
    ...
    for (int i = 0; i != num_relocs[core]; ++i) {
      if (relocs[core][i].type == code) {
        return &relocs[core][i];
      }
    }
  }
...
{
  if (reloc_offsets[core]) {
    // Currently a linear search, which isn't great...
    for (int i = 0; i != num_reloc_offsets[core]; ++i) {
      if (reloc_offsets[core][i].key == code) {
        if (reloc_offsets[core][i].value >= 0) {
          return reloc_offsets[core][i].value;
        } else {
...
bfd_reloc_code_real_type adl_get_reloc_type_by_name(const char *reloc_name) {
	...
	for (; reloc != reloc_end; reloc++) {
		if (!strcmp(reloc->name, reloc_name)) {
			return (bfd_reloc_code_real_type)relocs[core][reloc->index].type;
		}
	}
