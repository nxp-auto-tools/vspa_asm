...
reloc_howto_type *adl_elf_info_to_howto(unsigned);

/*
  Hack: We tap in here and call the ADL variant that's defined in the generated
        ADL disassembler.  We really should at some point remove any remaining
        PPC stuff from the generated assembler/disassembler and just have a
        generic "ADL" target for binutils.  
*/
static void
...
{
  reloc_howto_type *adl_howto = adl_elf_info_to_howto(ELF32_R_TYPE(dst->r_info));
  if (adl_howto) {
...
static bfd_boolean
ppc_elf_object_p (bfd *abfd ATTRIBUTE_UNUSED)
{
  // ADL:  It's OK for us to have a 64-bit architecture with a 32-bit ELF, at least for ADL.
# if 0
			  ...
			  asection **local_sections)
{
  ...
  // ADL:  Major hack here:  This is a simple linker driver designed to tie into the generated relocations from ADL.  It's pretty 
  // simple and just hacked into the PPC variant for simplicity.  At some point, we should clean all of this up!
  for (; rel < relend; rel++) {
    enum elf_ppc_reloc_type r_type;
    bfd_vma addend;
    ...
    howto = adl_elf_info_to_howto(ELF32_R_TYPE(rel->r_info));
...
/* changed to EM_VSPA from EM_PPC in order to be recognized as VSPA */
