...
/* True if mv rx,rxy between loads should be treated as errors.  */
COMMON int treat_mv_rx_ry_load_as_error; /* --treat_mv_rx_ry_load_as_error */
...
#define VPRINTF_LIKE(FCN)	void FCN (const char *format, va_list args)
#define VPRINTF_WHERE_LIKE(FCN)	void FCN (char *file, unsigned int line, const char *format, va_list args)
...
VPRINTF_LIKE (as_vbad);
VPRINTF_LIKE (as_vfatal) ATTRIBUTE_NORETURN;
VPRINTF_LIKE (as_vtsktsk);
VPRINTF_LIKE (as_vwarn);
