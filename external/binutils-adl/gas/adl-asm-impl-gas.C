    ...
    if (operand) {
      ...
      if (fixP->fx_size > 4) {
        if (target_big_endian) {
		  ...
		  // WA CMPVSPA-2302: For relocations with size < 32 bits
		  // e.g. R_VSPA_1 (8 bits), R_VSPA_2 (16 bits)
		  // the relocation should write only the size of the field.
		  if (fixP->fx_size == 1)
		  {
			  *(uint8_t *)instr = (uint8_t)(value & 0xff);
		  }
		  else if (fixP->fx_size == 2)
		  {
			  *(uint16_t *)instr = (uint16_t)(value & 0xffff);
		  }
		  else
		  {
			  *instr = value;
