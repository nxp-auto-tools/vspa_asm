...
/* If a symbol name does not exist, create it as undefined, and insert
   ...
   BCK: If stdoutput is null, then don't do anything.  This can occur when we we
         use the DevTech assembler, which uses its own symbol table.  */
symbolS *
	  ...
	  if (!(seg_left == absolute_section
		&& seg_right == absolute_section)
	      ...
	      && !((op == O_subtract
			...
			|| op == O_multiply || op == O_divide || op == O_modulus
			|| op == O_add || op == O_left_shift || op == O_right_shift
			|| op == O_bit_and || op == O_bit_inclusive_or
			|| op == O_bit_exclusive_or || op == O_logical_and
			|| op == O_logical_or)
		   && seg_left == seg_right
...
#define HANDLE_XADD_OPT2(str_) 						\
    ...
    case O_md1:          	HANDLE_XADD_OPT2 ("<<<");
    case O_md2:          	HANDLE_XADD_OPT2 (">>>");
