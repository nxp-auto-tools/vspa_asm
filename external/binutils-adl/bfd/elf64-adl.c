...
{
  reloc_howto_type *adl_howto = adl_elf_type_to_howto(ELF64_R_TYPE(dst->r_info));
  if (adl_howto) {
			  ...
			  asection **local_sections)
{
  ...
  for (; rel < relend; rel++) {
    ...
    if (r == bfd_reloc_other) {
      ...
      howto = adl_elf_type_to_howto(ELF64_R_TYPE(rel->r_info));
...

#define TARGET_LITTLE_SYM	adl_elf64_le_vec
#define TARGET_LITTLE_NAME	"elf64-adlle"
#define TARGET_BIG_SYM		adl_elf64_vec
#define TARGET_BIG_NAME		"elf64-adl"
...
#define bfd_elf64_bfd_reloc_type_lookup		adl_elf_reloc_type_lookup
#define bfd_elf64_bfd_reloc_name_lookup		adl_elf_reloc_name_lookup
#define bfd_elf64_bfd_relax_section		    acb_elf_relax_section
