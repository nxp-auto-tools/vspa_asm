...
/* level of nesting inside pairs of ld [aX]+aY and ld Rx */
static int between_loads = 0;
...
static void check_instr_bundle(InstrBundle &instr_bundle, bool &rts,
	...
	bool &dmem, bool &set_loop_instr, bool &loop_begin, bool &loop_end, bool &load, bool &mpy,
	bool &load_R, int &between_loads, struct loop_info *&li, struct coff_info &cinfo)
{
	bool reg;
	bool mvip, move, scalar_mac;
	size_t jmp_jsr_index, set_loop_index;

	// Scan the current instruction bundle to find control and DMEM access
	// instructions.
	check_control_dmem_instrs(instr_bundle, set_loop_instr, loop_begin, loop_end,
		set_loop_index, rts, jmp_jsr_index, jj_info, dmem, mvip, load, mpy, move, scalar_mac, load_R, between_loads);
...
static void write_instrs(void)
{
	...
	if (core == CORE_VCPU)
	{
		...
		bool rts, jmp_jsr, dmem, set_loop_instr, loop_begin, loop_end, load, mpy, load_R;
		struct coff_info cinfo;
		struct loop_info *li = NULL;
		struct jmp_jsr_info jj_info;
		check_instr_bundle(instrInfos[curgrp()], rts, jmp_jsr, jj_info, dmem,
			set_loop_instr, loop_begin, loop_end, load, mpy, load_R, between_loads, li, cinfo);
		...
		if (compact_instr(instrInfos[curgrp()], last_group))
		{
			...
			ss->update_section_flags(now_seg, rts, jmp_jsr, jj_info, dmem, cinfo,
				set_loop_instr, loop_begin, loop_end, load, mpy, li);
		}
		else
		{
			...
			else if (instrInfos[curgrp()].size())
			{
				ss->update_section_flags(now_seg, rts, jmp_jsr, jj_info, dmem,
					cinfo, set_loop_instr, loop_begin, loop_end, load, mpy, li);
			}
			else
			{
				delete li;
