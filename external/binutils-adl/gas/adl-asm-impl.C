...
extern "C" {
	...
	extern const struct option md_longopts[] __attribute__((weak)) = {
		...
		{ "debug-path", required_argument, NULL, OPTION_DEBUG_PATH },
...
struct section_flags
{
	...
	// "set.loop" instruction included in the previous bundle.
	bool prev_set_loop_instr;
	...
	bool prev_loop_end;
	//one instruction loop ending in the vles previous
	bool prev_one_inst_loop;
	//two instructions loop ending in the vles previous
	bool prev_two_inst_loop;
	//one or two instructions loop ending two vles previous
	bool prev2_one_inst_loop;
	//load in the vles previous
	bool prev_load;
	//load two vles previous
	bool prev2_load;
	//mpy in the previous vles
	bool prev_mpy;
	...
	section_flags() :
		...
		prev2_one_inst_loop(false),
		prev_one_inst_loop(false),
		prev_two_inst_loop(false),
		prev_mpy(false),
		prev_load(false),
		prev2_load(false)
	{
	...
	section_flags(const section_flags &sf) :
		...
		prev_set_loop_instr(sf.prev_set_loop_instr),
		...
		prev2_one_inst_loop(sf.prev2_one_inst_loop),
		prev_one_inst_loop(sf.prev_one_inst_loop),
		prev_two_inst_loop(sf.prev_two_inst_loop)
	{
	}
...
public:
	...
	void update_section_flags(segT sec, const bool &rts, const bool &jmp_jsr,
		...
		const bool &set_loop_instr, const bool &loop_begin, const bool &loop_end, const bool &load,
		const bool &mpy, const struct loop_info *li)
	{
		...
		sf->prev2_one_inst_loop = sf->prev_one_inst_loop;
		sf->prev2_load = sf->prev_load;
		
		if (loop_end && loop_begin)
		{
			sf->prev_one_inst_loop = true;
			sf->prev_two_inst_loop = false;
		}
		else if (loop_end && sf->prev_loop_begin)
		{
			sf->prev_one_inst_loop = false;
			sf->prev_two_inst_loop = true;
		}
		else
		{
			sf->prev_one_inst_loop = false;
		...
		sf->prev_set_loop_instr = set_loop_instr; 
...
void vspa_post_asm(unsigned width, int group)
{
	...
	check_instr_unavailable_modes(instr);
...
InstrInfo replaceInstr(const InstrInfo &instr,const string &new_name)
{
	//wa cmpvspa-2211
	bool add_cond = false;
	bool add_sp_first = false;
	bool add_sp_second = false;
	string name = new_name;
	if (new_name.compare("mvS_agX_agY_mv") == 0 || new_name.compare("mvS_aX_agY_mv") == 0)
	{
	...
	else if (new_name.compare("mvS_sp_aY_mv") == 0)
	{
		size_t pos = instr.str.find(',');
		if (instr.str[pos + 1] == 'g')
		{
			name = "mv_cc_gZ_gX";
			add_cond = true;
			add_sp_first = true;
	...
	else if (new_name.compare("mvS_aX_sp_mv") == 0 && instr.str[1]== 'g')
	{
		name = "mv_cc_gZ_gX";
		add_cond = true;
		add_sp_second = true;
	...
	if (new_op)
	{
		...
		if (add_sp_second){
			new_instr.operand_values[new_instr.num_operand_values() - 2].X_add_number = 14;
			new_instr.operand_values[new_instr.num_operand_values() - 2].X_op = O_constant;
		}
		if (add_cond)
		{
			expressionS ex = new_instr.operand_values[new_instr.num_operand_values() - 1];
...
static bool valid_input(struct adl_opcode *opcode, expressionS *operand_values)
{
  int i;
  for (i = 0; i != opcode->num_operands; ++i) {
    struct adl_operand *operand = &opcode->operands[i];
    expressionS *ex = &operand_values[operand->arg];
    ...
    if (ex->X_op == O_constant) {
	  bool old_check_low_bits = check_low_bits;
	  if (strstr(opcode->name, "wide_imm"))
	  {//don't check value for wide_imm constraints in this function, it will be checked further down
		check_low_bits = false;
	  }
	  if (core == CORE_IPPU && strncmp(operand->ptr->name, "IM12s_imp_bits",14) == 0)
	  {
		  if (AU_NUM == 16)
		  {
			  operand->lower = -512;
	  ...
	  check_low_bits = old_check_low_bits;
	  ...
	  if (strstr(opcode->name, "wide_imm"))
	  {
		  ...
		  if (!fitsSignedOnNbits(ex->X_add_number, 9)){
			  int val = ex->X_add_number / (AU_NUM * 8);
			  if (!fitsSignedOnNbits(val, 9)){
				  return false;
			  }
		  }
	  }
	...
	const bool &two_opS)
{
	...
	if (prev_jmp_jsr)
	{
		...
		if (ms.jmp_jsr && !ms.jj_info.cond)
		{
			if (ms.jj_info.jmp_mnemonic)
			{
...
//  Check delay slot restrictions.
static void check_smode_fft_restrictions(adl::InstrBundle &instr_bundle)
{
	for (unsigned int i = 0; i < instr_bundle.group_size(); i++)
	{
		const adl::InstrInfo &instr = instr_bundle.get_instr(i);
		if (strncmp(instr.instrName(), "set_Smode", 9) == 0)
		{
			for (unsigned int j = 0; j < instr.args.size(); j++)
			{
				if (instr.args[j] == "S0fft5" && AU_NUM <= 32)
				{
					// possible issue
					if (AU_NUM == 32)
						as_warn("possible illegal set.smode %s for %d AU, if single precision AU ", instr.args[j].c_str(), AU_NUM);
					// issue 
					else
						as_bad("found illegal set.smode %s for %d AU ", instr.args[j].c_str(), AU_NUM);
				}
				else if (instr.args[j] == "S0fft4" && AU_NUM <= 16)
				{
					// possible issue
					if (AU_NUM == 16)
						as_warn("possible illegal set.smode %s for %d AU, if single precision AU ", instr.args[j].c_str(), AU_NUM);
				...
				else if (instr.args[j] == "S0fft3" && AU_NUM <= 8)
				{
					// possible issue
					if (AU_NUM == 8)
						as_warn("possible illegal set.smode %s for %d AU, if single precision AU ", instr.args[j].c_str(), AU_NUM);
				...
				else if (instr.args[j] == "S0fft2" && AU_NUM <= 4)
				{
					// possible issue
					if (AU_NUM == 4)
						as_warn("possible illegal set.smode %s for %d AU, if single precision AU ", instr.args[j].c_str(), AU_NUM);
				...
				else if (instr.args[j] == "S0fft1" && AU_NUM == 2)
				{
					// possible issue
					as_warn("possible illegal set.smode %s for %d AU, if single precision AU ", instr.args[j].c_str(), AU_NUM);
				}
		...
		if (instr.src_opcode != NULL)
		{
			if (!strcmp(instr.src_opcode->name, "wr_fft7_fft2_fft7"))
			{
				if (AU_NUM == 64)
					// possible issue
					as_warn("possible illegal wr.fft7 for %d AU, if single precision wbType ", AU_NUM);
				else
					// issue
					as_bad("found illegal wr.fft7 for %d AU ", AU_NUM);
			}
			else if (!strcmp(instr.src_opcode->name, "wr_fn1_fft6_wr_fft6") && AU_NUM <= 32)
			{
				if (AU_NUM == 32)
					// possible issue
					as_warn("possible illegal wr.fft6 for %d AU, if single precision wbType ", AU_NUM);
				else
					// issue
					as_bad("found illegal wr.fft6 for %d AU ", AU_NUM);
			}
			else if (!strcmp(instr.src_opcode->name, "wr_fn_fft5_wr_fft5") && AU_NUM <= 16)
			{
				if (AU_NUM == 16)
					// possible issue
					as_warn("possible illegal wr.fft5 for %d AU, if single precision wbType ", AU_NUM);
				else
					// issue
					as_bad("found illegal wr.fft5 for %d AU ", AU_NUM);
			}
			else if (!strcmp(instr.src_opcode->name, "wr_straight_fft4_fft4") && AU_NUM <= 8)
			{
				if (AU_NUM == 8)
					// possible issue
					as_warn("possible illegal wr.fft4 for %d AU, if single precision wbType ", AU_NUM);
				else
					// issue
					as_bad("found illegal wr.fft4 for %d AU ", AU_NUM);
			}
			else if (!strcmp(instr.src_opcode->name, "wr_even_fft3_fft3") && AU_NUM <= 4)
			{
				if (AU_NUM == 4)
					// possible issue
					as_warn("possible illegal wr_fft3 for %d AU, if single precision wbType ", AU_NUM);
				else
					// issue
					as_bad("found illegal wr.fft3 for %d AU ", AU_NUM);
			}
			else if (!strcmp(instr.src_opcode->name, "wr_fft7_fft2_fft2") && AU_NUM == 8)
			{
				// possible issue
				as_warn("possible illegal wr_fft2 for %d AU, if single precision wbType ", AU_NUM);
			}
		}
	}
}
...
static void extract_coff_info(const InstrBundle &instr_bundle, struct coff_info &info)
{
	...
	if (ms.jmp_jsr_index == instr_bundle.group_size())
	{
		return;
	}
	...
	const InstrInfo &instr = instr_bundle.get_instr(ms.jmp_jsr_index);
...
static void check_instr_bundle(InstrBundle &instr_bundle, struct coff_info &cinfo)
{
	bool reg;

	// Scan the current instruction bundle to find control and DMEM access
	// instructions.
	check_control_dmem_instrs(instr_bundle, ms);
	...
	if ((sf->prev_one_inst_loop || sf->prev2_one_inst_loop) && ms.loop_end)
	{
		as_bad("found one or two instruction loop in the last three cycles of loop");
	}
	if (sf->prev_two_inst_loop  && ms.loop_end)
	{
		as_bad("found one or two instruction loop in the last three cycles of loop");
	}
	if (sf->prev_set_loop_instr && ms.loop_begin)
	{
		as_bad("found set_loop in the previous cycle of loop_begin");
	}
	if (ms.set_loop_instr && ms.loop_begin)
	{
		as_bad("found set_loop in the same cycle as loop_begin");
	}
	if (ms.between_loads && ms.move)
	{
		if (treat_mv_rx_ry_load_as_error)
			as_bad("found a mv Rx,Ry between load instructions");
		else
			as_warn("possible mv Rx,Ry between load instructions");
	}
	if (ms.load_R && ms.move)
	{
		as_bad("found a mv rx,ry in the same packet with a load R instructions");
	}
	if (sf->prev_mpy && ms.scalar_mac)
	{
		as_bad("Found a scalar mac instruction in the next cycle after a scalar mutiply");
	}
	...
	if (dmem_coff || dmem_mvip)
	{
		if (dmem_mvip)
		{
			check_dmem_ipwrite_inconsistency_linear(sf->prev2_dmem, ms.mvip);
		}
		check_dmem_cof_or_mvip_inconsistency(sf->prev2_dmem, ms.rts, ms.jmp_jsr, ms.loop_end, cinfo);

		// Check for delay slot DMEM access inconsistencies.
		check_dmem_cof_or_mvip_inconsistency(ms.dmem, sf->prev2_rts, sf->prev2_jmp_jsr,
			sf->prev2_coff_info, 2);
		check_dmem_cof_or_mvip_inconsistency(ms.dmem, sf->prev_rts, sf->prev_jmp_jsr,
			sf->prev_coff_info, 1);
		check_dmem_cof_or_mvip_inconsistency(sf->prev_dmem & sf->prev_loop_begin,
			sf->prev_loop_end, ms.rts, ms.jmp_jsr, cinfo, sf->prev_loop_info, ms.mvip);
	}

	// Extract loop specific information.
	if (ms.set_loop_index < instr_bundle.group_size())
	{
		extract_set_loop_info(instr_bundle, ms.set_loop_index);
	}

	// Increment control counter.
	sf->inc_control_counter(ms.jmp_jsr || ms.rts);

	// Check fft
	check_smode_fft_restrictions(instr_bundle);
}
...
static void write_instrs(void)
{
	...
	if (core == CORE_VCPU)
	{
		...
		check_mv_loadstore_restriction2(instrInfos[curgrp()]);

		check_ld_Vldb_restriction(instrInfos[curgrp()]);

		check_loadstore_restriction(instrInfos[curgrp()]);

		// Check for control instructions in the current bundle.
		struct coff_info cinfo;
		ms.li = NULL;
		check_instr_bundle(instrInfos[curgrp()], cinfo);
		...
		{
			dwarf2_emit_insn(8);
			ss->update_section_flags(now_seg, ms.rts, ms.jmp_jsr, ms.jj_info, ms.dmem, cinfo,
				ms.set_loop_instr, ms.loop_begin, ms.loop_end, ms.load, ms.mpy, ms.li);
		}
		else
		{
			...
			{
				ss->update_section_flags(now_seg, ms.rts, ms.jmp_jsr, ms.jj_info, ms.dmem,
					cinfo, ms.set_loop_instr, ms.loop_begin, ms.loop_end, ms.load, ms.mpy, ms.li);
			}
			else
			{
				delete ms.li;
                         ...
                         const char *instr_table_name,const string &eoi_str, bool is_laddr_inst) 
{
  ...
  if (opcode->asm_func) {
    ...
    for (int t_iter = 0; t_iter != opcode->num_alias_targets; ++t_iter) {
	  ...
	  bool laddr = false;
	  if ( (strstr(opcode->name, "line1") || strstr(opcode->name, "Line1")) && (trg->operands[1].leftshift == 9) && ((AU_NUM == 2) || (AU_NUM == 16)))
	  {
		  laddr = true;
	  }
...
bool restriction_checked_on_isa(void(*check_restr)(const adl::InstrBundle &), int isa_id)
{
	...
	// ERR050465 - SeaEagle retsriction
	if (check_restr == &check_loadstore_restriction)
	{
		/* restriction should be checked only for one VSPA3 target - SeaEagle (VSPA3_LA9358) */
		if (isa_id == VSPA3_LA9358)
			return true;
	}
	return false;
}
