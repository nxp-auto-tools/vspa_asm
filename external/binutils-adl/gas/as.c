...
Options:\n\
  ...
  fprintf (stream, _("\
  --treat_mv_rx_ry_load_as_error\n\
                          treat mv rx, ry between loads restriction as error (default false)\n"));
#ifdef HAVE_ITBL_CPU
  fprintf (stream, _("\
...
parse_args (int * pargc, char *** pargv)
{
  ...
  enum option_values
    {
      ...
      OPTION_TREAT_MV_RX_RY_AS_ERROR,
      OPTION_WARN,
      OPTION_TARGET_HELP,
      OPTION_EXECSTACK,
      OPTION_NOEXECSTACK,
  ...
  static const struct option std_longopts[] =
  {
    ...
    ,{"treat_mv_rx_ry_load_as_error", no_argument, NULL, OPTION_TREAT_MV_RX_RY_AS_ERROR}
  ...
  while (1)
	...
	case OPTION_TREAT_MV_RX_RY_AS_ERROR:
		treat_mv_rx_ry_load_as_error = 1;
		break;

	case OPTION_VERSION:
		md_parse_option('v', "version");
