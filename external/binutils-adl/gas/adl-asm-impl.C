...
unsigned adl_reloc_offset(bfd_reloc_code_real_type code, const struct adl_operand *oprnd, int instr_sz)
{
	...
	if (target_big_endian)
	{
		reloc_howto_type *reloc_howto = adl_reloc_type_lookup(stdoutput, code);
                         ...
                         const char *instr_table_name,const string &eoi_str, bool is_laddr_inst) 
{
  ...
  if (opcode->asm_func) {
    ...
    for (int t_iter = 0; t_iter != opcode->num_alias_targets; ++t_iter) {
	  ...
	  bool laddr = false;
	  if (strstr(opcode->name, "line1") && AU_NUM == 16 && trg->operands[1].leftshift == 9)
	  {
		  laddr = true;
	  }
	...
	//it will be removed when an adl fix will be available
		assert(opcode->operands[1].leftshift == 9 || opcode->operands[1].leftshift == 7);
		opcode->operands[1].leftshift = 7;
	}
	if (opcode->action) { 
...
bool instr_present_on_isa(const adl::InstrInfo &instr, int isa_id)
{
	...
	{
		enum target_parameters_enum val = (enum target_parameters_enum) (i - PARAM_ATTR_MIN);
