...
/* Hack:  Just as for 32-bit elf, we call out to the generated relocation lookup function.  If not found, then we
   continue using PPC's.  We really need to ditch the ppc stuff and just have a generic "ADL" backend. 
*/
static void
			 ...
			 Elf_Internal_Rela *dst)
{
  ...
  reloc_howto_type *adl_howto = adl_elf_info_to_howto(ELF64_R_TYPE(dst->r_info));
  if (adl_howto) {
...
static bfd_boolean
ppc64_elf_object_p (bfd *abfd ATTRIBUTE_UNUSED)
{
  // ADL:  It's OK for us to have a 32-bit architecture with a 64-bit ELF, at least for ADL.
# if 0
  ...
  if (r2off == 0)
    {
	 ...
	 opd entry.  */
      ...
      if (!htab->opd_abi) {
		...
		if (strcmp (opd->name, ".opd") != 0 || opd->reloc_count != 0)	{
			    ...
			    asection **local_sections)
{
  ...
  for (; rel < relend; rel++) {
    ...
    howto = adl_elf_info_to_howto(ELF64_R_TYPE(rel->r_info));
